...
import com.szubd.mcdfs_and_rspc.common.Constants;
import com.szubd.mcdfs_and_rspc.eum.SparkJobState;
import com.szubd.mcdfs_and_rspc.service.RSPcService;
import com.szubd.mcdfs_and_rspc.util.HttpUtils;
import com.szubd.mcdfs_and_rspc.util.PropertiesUtil;
...
@Service
public class RSPcServiceImpl implements RSPcService {
    private String LIVY_URL = "";

    private final Logger logger = LogManager.getLogger(this.getClass());

    @Autowired
    private RealTimeRSPcWebSocket realTimeRSPcWebSocket;
...
//    }
    ...
    public boolean runSparkJob(SparkJob sparkJob) {
        ...
        if(sparkJobID == 0){
            logger.error("可能由于应用程序名重复导致的错误");
            return false;
        }

        while (true) {
            SparkJobState sparkJobState = getBatchSessionState(sparkJobID);
            Map<String, Object> info = getSparkJobInfo(sparkJobID);
            Map appInfo = (Map) info.get("appInfo");
            realTimeRSPcWebSocket.sendAllMessage("\n查询任务ID为" + sparkJobID + "的任务详情:\n" + "任务名：" + info.get("name")+"\n提交者："
                    + info.get("proxyUser") + "\n状态：" + info.get("state") + "\nappId：" + info.get("appId")
                    + "\ndriver页面：" + appInfo.get("driverLogUrl") + "\nsparkUI：" + appInfo.get("sparkUiUrl"));
            System.out.println("\n查询任务ID为" + sparkJobID + "的任务详情:\n" + "任务名：" + info.get("name")+"\n提交者："
                    + info.get("proxyUser") + "\n状态：" + info.get("state") + "\nappId：" + info.get("appId")
                    + "\ndriver页面：" + appInfo.get("driverLogUrl") + "\nsparkUI：" + appInfo.get("sparkUiUrl"));
            switch (sparkJobState) {
                case SHUTTING_DOWN:
                    System.out.println("\n================================job关闭==================================\n");
                    System.out.println(getSparkJobLog(sparkJobID));
                    return false;
                case ERROR:
                    System.out.println("\n================================job错误==================================\n");
                    System.out.println(getSparkJobLog(sparkJobID));
    ...
    public int startSparkJob(SparkJob sparkJob) {
        ...
        String state = null;
        try {
            state = batchSession.getString(Constants.LIVY_SESSION_STATE);
        }catch (Exception e){
            System.out.println(batchSession);
            return 0;
        }
